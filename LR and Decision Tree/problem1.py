...
def compute_a(z):
    ...
    if z >= 100:
    	...
    	#a = 1.0 / (1 + np.e ** -z) #unsupported operand type(s) for ** or pow(): 'float' and 'matrix'
    	a = 1.0 / (1 + math.exp(-z))
    #########################################
    return a
...
def compute_L(a,y):
    ...
    if a == 0.:
    	if y == 0:
    		...
    		L = 1e5+1 #According to the test, we set the number less than float('inf')
    elif a == 1.:
    	if y == 1:
    		L = 0.
    	else:
...
def compute_dL_da(a, y):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    #from sympy import log,diff,Symbol (for the use of another solution)
    if a == 0 and y == 1:
        dL_da = -1e6 #According to the test, we set the number larger than float('inf')
    elif a == 1 and y == 0:
        dL_da  = 1e6 #According to the test, we set the number less than float('inf')
    else:
        if y == 0:
            dL_da = 1 / (1 - a)
        else:
            dL_da = - 1 / a
        # Another solution!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! but using this solution takes 55 seconds
    	#x = a                               
    	#a = Symbol('a')
    	#L = -y * log(a) - (1 - y) * log(1 - a)
    	#dL_da = float(L.diff(a).subs({a:x}))
        
        # Other derivation method
        '''
        def f(a):
            return -y * math.log(a) - (1 - y) * math.log(1 - a)
        dL_da = derivative(f, a)'''
        '''L = -y * math.log(a) - (1 - y) * math.log(1 - a)
            dL_da = np.gradient(L,a)'''
...
def predict(Xtest, w, b):
    ...
    for i, x in enumerate(Xtest):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        P[i,0] = compute_a(compute_z(x,w,b))
        if P[i,0]  >= 0.5:
       		Y[i] = 1
        #########################################
    return Y, P
