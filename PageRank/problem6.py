...
def import_A(filename ='network.csv'):
    ...
    A_pre = np.loadtxt(filename, dtype=np.float, delimiter=',')
    A = np.matrix(A_pre)
...
def score2rank(x):
    ...
    keys, values, sorted_ids = [], [], []
    for index, k in enumerate(x):
        keys.append(index)
        values.append(k)
    z = dict(zip(keys,values))
    sorted_z = OrderedDict((sorted(z.items(), key = lambda x:x[1], reverse = True)))
    sorted_ids = list(sorted_z.keys())
